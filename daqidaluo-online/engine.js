...
// 开局：房主可配置「初始风浪币」和「每人抵押物数量」
function startGame(room, config) {
  if (room.phase !== "lobby" || room.players.length < 2) return false;
  const cfg = config || {};
  const base = clampInt(cfg.coins, 100, 10, 100000);
  const coll = clampInt(cfg.collateral, 3, 0, 10);
  room.base = base;
  room.players.forEach((p) => {
    p.coins = base;
    p.debt = 0;
    p.collateral = drawCollateral(coll);
    p.alive = true;
    p.streak = 0; p.bestStreak = 0; p.peak = base; p.wipeouts = 0; p.borrows = 0;
  });
  room.turn = 0; room.pot = 0; room.round = 0; room.lastResult = null;
  room.phase = "playing";
  room.log = [];
  addLog(room, "—— 开局。每人 " + base + " 风浪币，" + coll + " 件抵押物 ——", "big");
  beginTurn(room);
  return true;
}
